...
- **智能多轮对话**：支持连续追问，AI 自动压缩对话历史保持上下文连贯
- **关联推荐**：图片化的搭配套餐、优惠组合
- **竞品对比**：双产品图文对照
- **语音交互（离线）**：按住说话，AI 语音回复，无需联网即可使用
- **通用咨询**：未识别商品也能基于通用知识进行专业解答
...
如果您已经部署过旧版本，按以下步骤升级到最新版：

```bash
# 1. 进入部署目录
cd smart-shopping-guide-agent/deployment/docker

# 2. 停止并删除旧容器
docker-compose down

# 3. 删除旧镜像（确保使用最新代码重新构建）
docker rmi docker-smart-guide-agent:latest 2>/dev/null || true

# 4. 重新构建并启动（会自动下载语音模型到镜像中）
docker-compose up --build -d

# 5. 查看日志确认启动成功
docker logs smart-guide-agent --tail 30
```

**升级验证**：当看到 `[ASR] 初始化成功` 和 `[TTS] 初始化成功` 日志时，说明离线语音已就绪。
...
- **语音追问**：按住麦克风按钮说话提问
- **文字追问**：输入框输入问题
- **智能记忆**：连续追问时 AI 会自动保留上下文，回答更连贯
- **关联推荐**：查看搭配产品和竞品对比

### 未识别商品怎么办

如果拍摄/上传的商品不在产品库中，系统会自动进入**通用咨询模式**：
- AI 会礼貌告知该商品暂未收录
- 基于通用购物知识为您提供专业建议
- 仍可继续追问，获得连贯的咨询服务
...
| 版本 | 更新内容 |
|------|----------|
| **v0.3.0** | **开箱即用离线语音** - Docker 镜像内置 Sherpa-ONNX ASR/TTS 模型，无需配置 API Key |
| **v0.3.0** | **对话记忆压缩** - 支持 10+ 轮连续对话，自动压缩历史保持上下文连贯 |
| **v0.3.0** | **通用咨询模式** - 未识别商品也能基于通用知识进行专业解答 |
| **v0.2.0** | 模型连接测试功能 - Console 支持测试 VLM/LLM/TTS/ASR 连接状态 |
| **v0.1.0** | MVP 版本 - 视觉识别、产品讲解、关联推荐、竞品对比 |
