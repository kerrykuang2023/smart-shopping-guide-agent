...
| 知识库 | **5 个签字笔 SKU**（已锁定，详见 §3.1.1） | P0 |
| 产品图片库 | 每 SKU 5-10 张图（主图 + 多角度 + 关联） | P0 |
...
> **主题：文具签字笔** —— 选择理由：随处可见、不易损坏、品牌差异明显、故事丰富、对应真实文具连锁/校园书店/办公直销 B2B 场景。

| # | SKU | 品牌 | 类型 | 价位 | 演示亮点 |
|---|-----|------|------|------|---------|
| 1 | 晨光优品 K35 | 晨光（中国） | 中性笔 0.5mm | ¥2.5 | 国民第一文具品牌 |
| 2 | 得力 S01 | 得力（中国） | 中性笔 0.5mm | ¥3 | 办公直销代表 |
| 3 | PILOT 百乐 G2 | PILOT（日本） | 中性笔 0.7mm | ¥18 | 进口高端代表 |
| 4 | 英雄 359 | 英雄（中国） | 钢笔 / 铱金笔尖 | ¥45 | 国货传统经典 |
| 5 | Stabilo Boss | Stabilo（德国） | 荧光笔 | ¥12 | 学习/手账场景 |

**SKU 选择策略**：
- **价格梯度**：¥2.5 → ¥45（18× 价差，演示 AI 不依赖价格识别）
- **品类多样**：3 中性笔 + 1 钢笔 + 1 荧光笔（演示细分品类识别能力）
- **国别覆盖**：中国 × 3 + 日本 × 1 + 德国 × 1（国际品牌识别）
- **演示对照客户**：晨光生活馆、九木杂物社、大学书店、商超文具区、礼品店

**演示道具准备**：
- 物理实物：5 支笔（任意办公室抽屉即可凑齐 4-5 支，最差只需 ¥80 全部购齐）
- 图片素材：每支笔 5 张实拍 + 2-3 张品牌/官方图（详见 [`knowledge/images/README.md`](../knowledge/images/README.md)）
- 知识库 YAML：5 份完整的 [`knowledge/products/*.yaml`](../knowledge/products/) 已就绪
...
00:50 │ 客户拿起自己的手机扫码
       ...
       │
01:00 │ 演示者：「您看，扫码即开。请您对准这支 PILOT 百乐 G2 拍照。」
       │ [演示者递上一支已准备好的 PILOT G2 中性笔]
       │
01:10 │ 客户对准 PILOT G2 拍照
       │ 客户手机：识别中...（0.5-1 秒）
       │
01:15 │ 客户手机显示：
       │ - PILOT 百乐 G2 主图（透明笔身可见墨量）
       │ - 价格 ¥18 / 0.7mm 黑色 / 日本进口
       │ - AI 智能讲解开始展开（带配图）：
       │   "这是 PILOT 百乐 G2，全球公认最顺滑的中性笔之一……"
       │
01:30 │ 演示者：「整个识别和讲解都是在我们的边缘盒子上完成，
       │         您的手机数据没有上传到任何云端，完全本地推理。」
       │
01:50 │ 演示者：「您可以追问，比如问 "和晨光 K35 比怎么样？"」
       │ 客户输入问题
       │ AI 流式回复对比信息：
       │   "PILOT G2 价格是晨光 K35 的 7 倍，但顺滑度和耐用度
       │    ……晨光 K35 性价比突出适合学生……" 
       │
...
| 产品实物 | **5 支签字笔**（晨光 K35 / 得力 S01 / PILOT G2 / 英雄 359 / Stabilo Boss），知识库已配置 |
...
| 知识库准备（5 支笔 SKU） | PM/内容 | 1.5 天 | YAML（已就绪） + 实拍图片 |
